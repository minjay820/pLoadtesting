...
`pLoadtesting` may integrate with or adapt to third-party load testing engines, web frameworks, databases, container tooling, and observability tools. These external technologies are not owned by `pLoadtesting`, and each is governed by its own independent licenses, terms of service, and proprietary copyrights.
...
* **Django and Django REST Framework**: Used by the Control Plane. Licensed separately by their respective upstream projects.
* **FastAPI, Starlette, Uvicorn, and Pydantic**: Used by the reference target services, target app suite, and Worker Agent. Licensed separately by their respective upstream projects.
* **Celery and Redis**: Used for Control Plane scheduling and broker workflows. Licensed separately by their respective upstream projects.
* **InfluxDB and Grafana**: Used for local observability and dashboarding in the compose stack. Licensed separately by their respective upstream projects.
* **Docker and Docker Compose**: Used for local runtime orchestration. Docker products and related components are governed by their own upstream licenses and terms.
...
* **No Proprietary Redistribution**: Optional enterprise engine adapters must rely on user-provided licensed installations and must not commit proprietary binaries, wrappers, credentials, or license materials to this repository.
