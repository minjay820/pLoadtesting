...
- Coverage is not yet strict one-to-one parity for every single `k6` profile. Where one engine has finer-grained profiles, the other engine may currently use a representative bounded profile for the same target family.
...
jmeter -n \
  ...
  -JTARGET_METHOD=GET \
  -JTARGET_QUERY='rate=0.5&deterministic=true&request_key=pass'
```
